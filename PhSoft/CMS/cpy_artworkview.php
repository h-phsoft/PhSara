...
$cpy_artwork_view = NULL; // Initialize page object first

class ccpy_artwork_view extends ccpy_artwork {
	...
	// Page object name
	var $PageObjName = 'cpy_artwork_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["art_id"] <> "") {
			$this->RecKey["art_id"] = $_GET["art_id"];
			$KeyUrl .= "&amp;art_id=" . urlencode($this->RecKey["art_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["art_id"] <> "") {
				...
				$this->RecKey["art_id"] = $this->art_id->QueryStringValue;
			} elseif (@$_POST["art_id"] <> "") {
				$this->art_id->setFormValue($_POST["art_id"]);
				$this->RecKey["art_id"] = $this->art_id->FormValue;
			} else {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "cpy_artworklist.php"; // No matching record, return to list
					} else {
						$this->LoadRowValues($this->Recordset); // Load row values
					}
			}
		} else {
			$sReturnUrl = "cpy_artworklist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		$item = &$option->Add("detail_cpy_artwork_subject");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("cpy_artwork_subject", "TblCaption");
		...
		$links = "";
		if ($GLOBALS["cpy_artwork_subject_grid"] && $GLOBALS["cpy_artwork_subject_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'cpy_artwork_subject')) {
			$links .= "<li><a class=\"ewRowLink ewDetailView\" data-action=\"view\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailViewLink")) . "\" href=\"" . ew_HtmlEncode($this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=cpy_artwork_subject")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailViewLink")) . "</a></li>";
			if ($DetailViewTblVar <> "") $DetailViewTblVar .= ",";
			$DetailViewTblVar .= "cpy_artwork_subject";
		}
		if ($GLOBALS["cpy_artwork_subject_grid"] && $GLOBALS["cpy_artwork_subject_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'cpy_artwork_subject')) {
			$links .= "<li><a class=\"ewRowLink ewDetailEdit\" data-action=\"edit\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailEditLink")) . "\" href=\"" . ew_HtmlEncode($this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=cpy_artwork_subject")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailEditLink")) . "</a></li>";
			if ($DetailEditTblVar <> "") $DetailEditTblVar .= ",";
			$DetailEditTblVar .= "cpy_artwork_subject";
		}
		if ($GLOBALS["cpy_artwork_subject_grid"] && $GLOBALS["cpy_artwork_subject_grid"]->DetailAdd && $Security->CanAdd() && $Security->AllowAdd(CurrentProjectID() . 'cpy_artwork_subject')) {
			$links .= "<li><a class=\"ewRowLink ewDetailCopy\" data-action=\"add\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailCopyLink")) . "\" href=\"" . ew_HtmlEncode($this->GetCopyUrl(EW_TABLE_SHOW_DETAIL . "=cpy_artwork_subject")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailCopyLink")) . "</a></li>";
			if ($DetailCopyTblVar <> "") $DetailCopyTblVar .= ",";
		...
		$body = "<div class=\"btn-group\">" . $body . "</div>";
		$item->Body = $body;
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'cpy_artwork_subject');
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("cpy_artwork_subject", $DetailTblVar)) {
				...
				if ($GLOBALS["cpy_artwork_subject_grid"]->DetailView) {
					$GLOBALS["cpy_artwork_subject_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($cpy_artwork_view)) $cpy_artwork_view = new ccpy_artwork_view();

// Page init
$cpy_artwork_view->Page_Init();

// Page main
$cpy_artwork_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cpy_artwork_view->Page_Render();
?>
...
var CurrentForm = fcpy_artworkview = new ew_Form("fcpy_artworkview", "view");

// Form_CustomValidate event
fcpy_artworkview.Form_CustomValidate = 
...
fcpy_artworkview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fcpy_artworkview.Lists["x_type_id"] = {"LinkField":"x_type_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_type_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_arttype"};
fcpy_artworkview.Lists["x_type_id"].Data = "<?php echo $cpy_artwork_view->type_id->LookupFilterQuery(FALSE, "view") ?>";
...
<div class="ewToolbar">
<?php $cpy_artwork_view->ExportOptions->Render("body") ?>
<?php
	foreach ($cpy_artwork_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php $cpy_artwork_view->ShowPageHeader(); ?>
<?php
$cpy_artwork_view->ShowMessage();
?>
<?php if (!$cpy_artwork_view->IsModal) { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($cpy_artwork_view->Pager)) $cpy_artwork_view->Pager = new cPrevNextPager($cpy_artwork_view->StartRec, $cpy_artwork_view->DisplayRecs, $cpy_artwork_view->TotalRecs, $cpy_artwork_view->AutoHidePager) ?>
<?php if ($cpy_artwork_view->Pager->RecordCount > 0 && $cpy_artwork_view->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($cpy_artwork_view->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $cpy_artwork_view->PageUrl() ?>start=<?php echo $cpy_artwork_view->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($cpy_artwork_view->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $cpy_artwork_view->PageUrl() ?>start=<?php echo $cpy_artwork_view->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $cpy_artwork_view->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($cpy_artwork_view->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $cpy_artwork_view->PageUrl() ?>start=<?php echo $cpy_artwork_view->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($cpy_artwork_view->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $cpy_artwork_view->PageUrl() ?>start=<?php echo $cpy_artwork_view->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $cpy_artwork_view->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<form name="fcpy_artworkview" id="fcpy_artworkview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cpy_artwork_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cpy_artwork_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="cpy_artwork">
<input type="hidden" name="modal" value="<?php echo intval($cpy_artwork_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($cpy_artwork->art_year->Visible) { // art_year ?>
	<tr id="r_art_year">
		<td class="col-sm-2"><span id="elh_cpy_artwork_art_year"><?php echo $cpy_artwork->art_year->FldCaption() ?></span></td>
...
<?php echo $cpy_artwork->art_year->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($cpy_artwork->type_id->Visible) { // type_id ?>
	<tr id="r_type_id">
		<td class="col-sm-2"><span id="elh_cpy_artwork_type_id"><?php echo $cpy_artwork->type_id->FldCaption() ?></span></td>
...
<?php echo $cpy_artwork->type_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($cpy_artwork->art_title1->Visible) { // art_title1 ?>
	<tr id="r_art_title1">
		<td class="col-sm-2"><span id="elh_cpy_artwork_art_title1"><?php echo $cpy_artwork->art_title1->FldCaption() ?></span></td>
...
<?php echo $cpy_artwork->art_title1->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($cpy_artwork->art_title2->Visible) { // art_title2 ?>
	<tr id="r_art_title2">
		<td class="col-sm-2"><span id="elh_cpy_artwork_art_title2"><?php echo $cpy_artwork->art_title2->FldCaption() ?></span></td>
...
<?php echo $cpy_artwork->art_title2->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($cpy_artwork->art_size->Visible) { // art_size ?>
	<tr id="r_art_size">
		<td class="col-sm-2"><span id="elh_cpy_artwork_art_size"><?php echo $cpy_artwork->art_size->FldCaption() ?></span></td>
...
<?php echo $cpy_artwork->art_size->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($cpy_artwork->art_image->Visible) { // art_image ?>
	<tr id="r_art_image">
		<td class="col-sm-2"><span id="elh_cpy_artwork_art_image"><?php echo $cpy_artwork->art_image->FldCaption() ?></span></td>
		<td data-name="art_image"<?php echo $cpy_artwork->art_image->CellAttributes() ?>>
<span id="el_cpy_artwork_art_image">
<span>
<?php echo ew_GetFileViewTag($cpy_artwork->art_image, $cpy_artwork->art_image->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php } ?>
<?php
	if (in_array("cpy_artwork_exhibtion", explode(",", $cpy_artwork->getCurrentDetailTable())) && $cpy_artwork_exhibtion->DetailView) {
?>
...
<?php } ?>
<?php
	if (in_array("cpy_artwork_subject", explode(",", $cpy_artwork->getCurrentDetailTable())) && $cpy_artwork_subject->DetailView) {
?>
<?php if ($cpy_artwork->getCurrentDetailTable() <> "") { ?>
...
<script type="text/javascript">
fcpy_artworkview.Init();
</script>
<?php
$cpy_artwork_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cpy_artwork_view->Page_Terminate();
?>
