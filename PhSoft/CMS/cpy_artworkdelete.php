...
$cpy_artwork_delete = NULL; // Initialize page object first

class ccpy_artwork_delete extends ccpy_artwork {
	...
	// Page object name
	var $PageObjName = 'cpy_artwork_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("cpy_artworklist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in cpy_artwork class, cpy_artworkinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("cpy_artworklist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($cpy_artwork_delete)) $cpy_artwork_delete = new ccpy_artwork_delete();

// Page init
$cpy_artwork_delete->Page_Init();

// Page main
$cpy_artwork_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cpy_artwork_delete->Page_Render();
?>
...
var CurrentForm = fcpy_artworkdelete = new ew_Form("fcpy_artworkdelete", "delete");

// Form_CustomValidate event
fcpy_artworkdelete.Form_CustomValidate = 
...
fcpy_artworkdelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fcpy_artworkdelete.Lists["x_type_id"] = {"LinkField":"x_type_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_type_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_arttype"};
fcpy_artworkdelete.Lists["x_type_id"].Data = "<?php echo $cpy_artwork_delete->type_id->LookupFilterQuery(FALSE, "delete") ?>";
...
<?php $cpy_artwork_delete->ShowPageHeader(); ?>
<?php
$cpy_artwork_delete->ShowMessage();
?>
<form name="fcpy_artworkdelete" id="fcpy_artworkdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cpy_artwork_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cpy_artwork_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="cpy_artwork">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($cpy_artwork_delete->RecKeys as $key) { ?>
...
<?php if ($cpy_artwork->art_year->Visible) { // art_year ?>
		<th class="<?php echo $cpy_artwork->art_year->HeaderCellClass() ?>"><span id="elh_cpy_artwork_art_year" class="cpy_artwork_art_year"><?php echo $cpy_artwork->art_year->FldCaption() ?></span></th>
<?php } ?>
<?php if ($cpy_artwork->type_id->Visible) { // type_id ?>
		<th class="<?php echo $cpy_artwork->type_id->HeaderCellClass() ?>"><span id="elh_cpy_artwork_type_id" class="cpy_artwork_type_id"><?php echo $cpy_artwork->type_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($cpy_artwork->art_title1->Visible) { // art_title1 ?>
		<th class="<?php echo $cpy_artwork->art_title1->HeaderCellClass() ?>"><span id="elh_cpy_artwork_art_title1" class="cpy_artwork_art_title1"><?php echo $cpy_artwork->art_title1->FldCaption() ?></span></th>
<?php } ?>
<?php if ($cpy_artwork->art_title2->Visible) { // art_title2 ?>
		<th class="<?php echo $cpy_artwork->art_title2->HeaderCellClass() ?>"><span id="elh_cpy_artwork_art_title2" class="cpy_artwork_art_title2"><?php echo $cpy_artwork->art_title2->FldCaption() ?></span></th>
<?php } ?>
<?php if ($cpy_artwork->art_size->Visible) { // art_size ?>
		<th class="<?php echo $cpy_artwork->art_size->HeaderCellClass() ?>"><span id="elh_cpy_artwork_art_size" class="cpy_artwork_art_size"><?php echo $cpy_artwork->art_size->FldCaption() ?></span></th>
<?php } ?>
<?php if ($cpy_artwork->art_image->Visible) { // art_image ?>
		<th class="<?php echo $cpy_artwork->art_image->HeaderCellClass() ?>"><span id="elh_cpy_artwork_art_image" class="cpy_artwork_art_image"><?php echo $cpy_artwork->art_image->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$cpy_artwork_delete->RecCnt = 0;
$i = 0;
while (!$cpy_artwork_delete->Recordset->EOF) {
	$cpy_artwork_delete->RecCnt++;
	$cpy_artwork_delete->RowCnt++;

	// Set row properties
	$cpy_artwork->ResetAttrs();
	$cpy_artwork->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$cpy_artwork_delete->LoadRowValues($cpy_artwork_delete->Recordset);

	// Render row
	$cpy_artwork_delete->RenderRow();
?>
	<tr<?php echo $cpy_artwork->RowAttributes() ?>>
<?php if ($cpy_artwork->art_year->Visible) { // art_year ?>
		<td<?php echo $cpy_artwork->art_year->CellAttributes() ?>>
<span id="el<?php echo $cpy_artwork_delete->RowCnt ?>_cpy_artwork_art_year" class="cpy_artwork_art_year">
...
<span id="el<?php echo $cpy_artwork_delete->RowCnt ?>_cpy_artwork_type_id" class="cpy_artwork_type_id">
...
<span id="el<?php echo $cpy_artwork_delete->RowCnt ?>_cpy_artwork_art_title1" class="cpy_artwork_art_title1">
...
<span id="el<?php echo $cpy_artwork_delete->RowCnt ?>_cpy_artwork_art_title2" class="cpy_artwork_art_title2">
...
<span id="el<?php echo $cpy_artwork_delete->RowCnt ?>_cpy_artwork_art_size" class="cpy_artwork_art_size">
...
<span id="el<?php echo $cpy_artwork_delete->RowCnt ?>_cpy_artwork_art_image" class="cpy_artwork_art_image">
<span>
...
<?php
	$cpy_artwork_delete->Recordset->MoveNext();
}
$cpy_artwork_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cpy_artwork_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fcpy_artworkdelete.Init();
</script>
<?php
$cpy_artwork_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cpy_artwork_delete->Page_Terminate();
?>
