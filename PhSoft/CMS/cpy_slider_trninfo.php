...
class ccpy_slider_trn extends cTable {
	...
	function __construct() {
		...
		// slid_image
		$this->slid_image = new cField('cpy_slider_trn', 'cpy_slider_trn', 'x_slid_image', 'slid_image', '`slid_image`', '`slid_image`', 200, -1, TRUE, '`slid_image`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->slid_image->Sortable = TRUE; // Allow sort
		$this->fields['slid_image'] = &$this->slid_image;
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->slid_image->Upload->DbValue = $rs->fields('slid_image');
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->slid_image->Exportable) $Doc->ExportCaption($this->slid_image);
				} else {
					if ($this->slid_id->Exportable) $Doc->ExportCaption($this->slid_id);
					if ($this->slid_order->Exportable) $Doc->ExportCaption($this->slid_order);
					if ($this->slid_image->Exportable) $Doc->ExportCaption($this->slid_image);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->slid_image->Exportable) $Doc->ExportField($this->slid_image);
					} else {
						if ($this->slid_id->Exportable) $Doc->ExportField($this->slid_id);
						if ($this->slid_order->Exportable) $Doc->ExportField($this->slid_order);
						if ($this->slid_image->Exportable) $Doc->ExportField($this->slid_image);
