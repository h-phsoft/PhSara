...
$cpy_page_images_delete = NULL; // Initialize page object first

class ccpy_page_images_delete extends ccpy_page_images {
	...
	// Page object name
	var $PageObjName = 'cpy_page_images_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("cpy_page_imageslist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in cpy_page_images class, cpy_page_imagesinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("cpy_page_imageslist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($cpy_page_images_delete)) $cpy_page_images_delete = new ccpy_page_images_delete();

// Page init
$cpy_page_images_delete->Page_Init();

// Page main
$cpy_page_images_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cpy_page_images_delete->Page_Render();
?>
...
var CurrentForm = fcpy_page_imagesdelete = new ew_Form("fcpy_page_imagesdelete", "delete");

// Form_CustomValidate event
fcpy_page_imagesdelete.Form_CustomValidate = 
...
fcpy_page_imagesdelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fcpy_page_imagesdelete.Lists["x_page_id"] = {"LinkField":"x_page_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_page_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"cpy_page"};
fcpy_page_imagesdelete.Lists["x_page_id"].Data = "<?php echo $cpy_page_images_delete->page_id->LookupFilterQuery(FALSE, "delete") ?>";
...
<?php $cpy_page_images_delete->ShowPageHeader(); ?>
<?php
$cpy_page_images_delete->ShowMessage();
?>
<form name="fcpy_page_imagesdelete" id="fcpy_page_imagesdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cpy_page_images_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cpy_page_images_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="cpy_page_images">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($cpy_page_images_delete->RecKeys as $key) { ?>
...
<?php if ($cpy_page_images->page_id->Visible) { // page_id ?>
		<th class="<?php echo $cpy_page_images->page_id->HeaderCellClass() ?>"><span id="elh_cpy_page_images_page_id" class="cpy_page_images_page_id"><?php echo $cpy_page_images->page_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($cpy_page_images->page_order->Visible) { // page_order ?>
		<th class="<?php echo $cpy_page_images->page_order->HeaderCellClass() ?>"><span id="elh_cpy_page_images_page_order" class="cpy_page_images_page_order"><?php echo $cpy_page_images->page_order->FldCaption() ?></span></th>
<?php } ?>
<?php if ($cpy_page_images->page_image->Visible) { // page_image ?>
		<th class="<?php echo $cpy_page_images->page_image->HeaderCellClass() ?>"><span id="elh_cpy_page_images_page_image" class="cpy_page_images_page_image"><?php echo $cpy_page_images->page_image->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$cpy_page_images_delete->RecCnt = 0;
$i = 0;
while (!$cpy_page_images_delete->Recordset->EOF) {
	$cpy_page_images_delete->RecCnt++;
	$cpy_page_images_delete->RowCnt++;

	// Set row properties
	$cpy_page_images->ResetAttrs();
	$cpy_page_images->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$cpy_page_images_delete->LoadRowValues($cpy_page_images_delete->Recordset);

	// Render row
	$cpy_page_images_delete->RenderRow();
?>
	<tr<?php echo $cpy_page_images->RowAttributes() ?>>
<?php if ($cpy_page_images->page_id->Visible) { // page_id ?>
		<td<?php echo $cpy_page_images->page_id->CellAttributes() ?>>
<span id="el<?php echo $cpy_page_images_delete->RowCnt ?>_cpy_page_images_page_id" class="cpy_page_images_page_id">
...
<?php } ?>
<?php if ($cpy_page_images->page_order->Visible) { // page_order ?>
		<td<?php echo $cpy_page_images->page_order->CellAttributes() ?>>
<span id="el<?php echo $cpy_page_images_delete->RowCnt ?>_cpy_page_images_page_order" class="cpy_page_images_page_order">
...
<?php } ?>
<?php if ($cpy_page_images->page_image->Visible) { // page_image ?>
		<td<?php echo $cpy_page_images->page_image->CellAttributes() ?>>
<span id="el<?php echo $cpy_page_images_delete->RowCnt ?>_cpy_page_images_page_image" class="cpy_page_images_page_image">
<span>
...
<?php
	$cpy_page_images_delete->Recordset->MoveNext();
}
$cpy_page_images_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cpy_page_images_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fcpy_page_imagesdelete.Init();
</script>
<?php
$cpy_page_images_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cpy_page_images_delete->Page_Terminate();
?>
