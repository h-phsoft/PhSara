...
$cpy_exhibition_video = NULL;

//
// Table class for cpy_exhibition_video
//
class ccpy_exhibition_video extends cTable {
	var $vexhib_id;
	var $exhib_id;
	var $video_id;
	...
	function __construct() {
		...
		$this->TableVar = 'cpy_exhibition_video';
		$this->TableName = 'cpy_exhibition_video';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`cpy_exhibition_video`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// vexhib_id
		$this->vexhib_id = new cField('cpy_exhibition_video', 'cpy_exhibition_video', 'x_vexhib_id', 'vexhib_id', '`vexhib_id`', '`vexhib_id`', 3, -1, FALSE, '`vexhib_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->vexhib_id->Sortable = FALSE; // Allow sort
		$this->vexhib_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['vexhib_id'] = &$this->vexhib_id;

		// exhib_id
		$this->exhib_id = new cField('cpy_exhibition_video', 'cpy_exhibition_video', 'x_exhib_id', 'exhib_id', '`exhib_id`', '`exhib_id`', 3, -1, FALSE, '`exhib_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// video_id
		$this->video_id = new cField('cpy_exhibition_video', 'cpy_exhibition_video', 'x_video_id', 'video_id', '`video_id`', '`video_id`', 3, -1, FALSE, '`video_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`cpy_exhibition_video`";
	}
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`exhib_id` ASC,`video_id` ASC";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->vexhib_id->setDbValue($conn->Insert_ID());
			$rs['vexhib_id'] = $this->vexhib_id->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('vexhib_id', $rs))
				ew_AddFilter($where, ew_QuotedName('vexhib_id', $this->DBID) . '=' . ew_QuotedValue($rs['vexhib_id'], $this->vexhib_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`vexhib_id` = @vexhib_id@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->vexhib_id->CurrentValue))
			return "0=1"; // Invalid key
		if (is_null($this->vexhib_id->CurrentValue))
			return "0=1"; // Invalid key
		else
			$sKeyFilter = str_replace("@vexhib_id@", ew_AdjustSql($this->vexhib_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "cpy_exhibition_videolist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "cpy_exhibition_videoedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "cpy_exhibition_videoadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("cpy_exhibition_videoview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("cpy_exhibition_videoview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "cpy_exhibition_videoadd.php?" . $this->UrlParm($parm);
		else
			$url = "cpy_exhibition_videoadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("cpy_exhibition_videoedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("cpy_exhibition_videoadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("cpy_exhibition_videodelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "vexhib_id:" . ew_VarToJson($this->vexhib_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->vexhib_id->CurrentValue)) {
			$sUrl .= "vexhib_id=" . urlencode($this->vexhib_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["vexhib_id"]))
				$arKeys[] = $_POST["vexhib_id"];
			elseif (isset($_GET["vexhib_id"]))
				$arKeys[] = $_GET["vexhib_id"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->vexhib_id->CurrentValue = $key;
	...
	function LoadListRowValues(&$rs) {
		$this->vexhib_id->setDbValue($rs->fields('vexhib_id'));
	...
	// Common render codes
		...
		$this->vexhib_id->ViewValue = $this->vexhib_id->CurrentValue;
		$this->vexhib_id->ViewCustomAttributes = "";
		...
		// vexhib_id
		$this->vexhib_id->LinkCustomAttributes = "";
		$this->vexhib_id->HrefValue = "";
		$this->vexhib_id->TooltipValue = "";
	...
	function RenderEditRow() {
		...
		// vexhib_id
		$this->vexhib_id->EditAttrs["class"] = "form-control";
		$this->vexhib_id->EditCustomAttributes = "";
		$this->vexhib_id->EditValue = $this->vexhib_id->CurrentValue;
		$this->vexhib_id->ViewCustomAttributes = "";
